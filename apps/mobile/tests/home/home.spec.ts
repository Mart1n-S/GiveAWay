...
test.describe("Page Accueil — Vue Marketing (non authentifié)", () => {
  test("devrait afficher la vue marketing pour un visiteur non authentifié", async ({
    ...
    await expect(page.getByText(/trouvez des missions qui/i).first()).toBeVisible();
  });
  ...
  test("devrait afficher le titre principal de la landing page", async ({
    ...
    await expect(
      page.getByText(/trouvez des missions qui/i),
    ).toBeVisible();
  });
  ...
  test("devrait afficher les étapes du processus bénévole", async ({
    ...
    await expect(page.getByText(/engagez-vous près de chez vous/i)).toBeVisible();
  });

  test("devrait afficher la section 'Pourquoi GiveAWay'", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("devrait afficher le bouton 'Rejoindre GiveAWay'", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("devrait naviguer vers /inscription au clic sur 'Rejoindre GiveAWay'", async ({
    ...
    await page
      .getByRole("button", { name: /rejoindre giveaway/i })
      .click();
